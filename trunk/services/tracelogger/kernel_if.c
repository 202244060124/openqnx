...
static int attach_to_paddr(int nbufs, paddr_t paddr)
{
    ...
    if (num_colours > 0) {
        ...
        vaddr = (unsigned)mmap(0, (nbufs * _TRACEBUFSIZE) + (num_colours * PAGESIZE), PROT_READ | PROT_WRITE,
                               MAP_PRIVATE | MAP_LAZY | MAP_ANON, NOFD, 0);
    ...
    kernel_buffers_vaddr = mmap((void*)vaddr, nbufs * _TRACEBUFSIZE, PROT_READ | PROT_WRITE,
                                vaddr == 0 ? /* only use fixed mapping if we are worried about colours */
                                    (MAP_SHARED | MAP_PHYS)
                                           : (MAP_FIXED | MAP_SHARED | MAP_PHYS),
                                NOFD, paddr);
