...
uintptr_t (*vaddr_search_adjust_hook)(uintptr_t vaddr, ADDRESS* adp, unsigned flags, uintptr_t size);
...
int tymem_pmem_alloc(PROCESS* prp, OBJECT* obp, struct map_set* ms, struct mm_map* mm, unsigned flags, off64_t boff, size_t size)
{
    int r;
    ...
    if ((pq->blk != PAQ_BLK_FAKE) || (pq->flags & PAQ_FLAG_RDB)) {
        ...
        for (pq_add = pq; pq_add != NULL; pq_add = pq_next) {
            ...
            r = memobj_pmem_walk(MPW_PHYS, obp, mm->offset, mm->offset + run_len - 1, tymem_split, &data);
            if (r != EOK)
                goto fail1;
...
static int rdecl setup_mappings(PROCESS* prp, struct map_set* ms, unsigned flags, unsigned preload)
{
    ...
    if (flags & MAP_STACK) {
        ...
        r = memory_reference(&mm, start, end, (mm->mmap_flags & PROT_WRITE) ? (MR_WRITE | MR_TRUNC) : MR_TRUNC, ms);
    } else {
...
int vmm_mmap(PROCESS* prp, uintptr_t vaddr_requested, size_t size_requested, int prot, int flags, OBJECT* obp, uint64_t boff,
             unsigned alignval, unsigned preload, int fd, void** vaddrp, size_t* sizep, part_id_t mpart_id)
{
    uintptr_t vaddr;
    size_t size;
    size_t guardsize = 0;
    ...
    if (prp == NULL) {
        ...
        if ((obp != NULL) && (obp->hdr.type == OBJECT_MEM_SHARED) && (obp->mem.mm.flags & SHMCTL_PHYS)
        ...
        if (flags & MAP_ANON) {
            ...
            pq = pa_alloc(size_requested, 0, PAQ_COLOUR_NONE, PAA_FLAG_CONTIG, NULL, restrict_proc, resv);
            if (pq == NULL) {
    ...
    if ((obp != NULL) && ((flags & (MAP_TYPE | MAP_ANON | MAP_ELF | MAP_FIXED)) == (MAP_PRIVATE | MAP_ELF | MAP_FIXED))
        ...
        switch (obp->hdr.type) {
            ...
            case OBJECT_MEM_SHARED:
                ...
                if (vaddr < end_vaddr) {
                    ...
                    r = vmm_mmap(prp, vaddr, end_vaddr - vaddr, prot & PROT_MASK, MAP_PRIVATE | MAP_ANON | MAP_FIXED, NULL, 0, 0, 0, NOFD,
                                 &kludge_vaddr, &kludge_size, kludge_mpart_id);
                    if (r != EOK)
                        return r;
    ...
    if ((obp != NULL) && ((obp->hdr.type == OBJECT_MEM_SHARED) || (obp->hdr.type == OBJECT_MEM_TYPED))
        ...
        && ((obp->mem.mm.flags & SHMCTL_GLOBAL) || (obp->mem.mm.flags & (SHMCTL_ANON | SHMCTL_PHYS)) == SHMCTL_PHYS)
        ...
        if (global_vaddr == VA_INVALID) {
            ...
            if (obp->mem.mm.flags & SHMCTL_GLOBAL) {
                ...
                r = map_create(&ms, &ms_repl, &gbl_map, 0, ROUNDUP(obp->mem.mm.size, __PAGESIZE), ADDR_PAGE((1024 * 1024) - 1), 0);
            } else {
                // RUSH3: ARMism (1 megabyte vaddr alignment)
                r = map_create(&ms, &ms_repl, &gbl_map, 0, ROUNDUP(size_requested, __PAGESIZE), ADDR_PAGE((1024 * 1024) - 1), 0);
            }
            if (r != EOK) {
                pthread_mutex_unlock(&gbl_mux);
                goto fail0;
    ...
    if ((mask == 0) && (vaddr_requested == 0) && (mm_flags & MM_FLAG_VPS)) {
        ...
        switch (CPU_SYSTEM_HAVE_MULTIPLE_PAGESIZES) {
            case VPS_HIGHUSAGE:
                ...
                if ((obp == NULL) || (obp->hdr.type != OBJECT_MEM_SHARED) || !(obp->mem.mm.flags & SHMCTL_HIGHUSAGE))
                    break;
                // Fall through
            case VPS_AUTO:
