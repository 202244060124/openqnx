        ...
        if (this_start < this_end) {
            ...
            if (pq->blk == PAQ_BLK_FAKE) {
                ...
                if (pq->flags & PAQ_FLAG_MODIFIED) {
                    MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr), off, off + (num << QUANTUM_BITS) - 1, NULL, NULL);
                }
                ...
                if ((skip != 0) && (this_end < run_end)) {
                    ...
                    tmp = pa_alloc_fake(paddr + ((num + skip) << QUANTUM_BITS), (pq->run - (num + skip)) << QUANTUM_BITS);
                    if (tmp == NULL)
                        return ENOMEM;
                ...
                for (i = 0; i < num; ++i, ++tmp) {
                    if (tmp->flags & PAQ_FLAG_HAS_SYNC) {
                        ...
                        off = PADDR_TO_SYNC_OFF(paddr);
                        MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr), off, off + (QUANTUM_SIZE - 1), NULL, NULL);
                    }
                    paddr += QUANTUM_SIZE;
                }
...
static int pq_insert(OBJECT* obp, off64_t off, unsigned numq, struct pa_quantum* (*func)(OBJECT*, unsigned, void*), void* d)
{
    struct pa_quantum** owner;
...
static struct pa_quantum* pmem_alloc(OBJECT* obp, unsigned numq, void* d)
{
    ...
    pq = pa_alloc(NQUANTUM_TO_LEN(numq), 0, PAQ_COLOUR_NONE, data->flags, &status, data->restriction, resv);
    if (pq == NULL) {
...
int memobj_pmem_split(OBJECT* obp, struct pa_quantum* pq, unsigned split_quanta)
{
    ...
    new = pa_alloc_fake(pa_quantum_to_paddr(pq) + NQUANTUM_TO_LEN(split_quanta), NQUANTUM_TO_LEN(remaining));
    if (new == NULL)
        return ENOMEM;
...
int memobj_pmem_walk(int flags, OBJECT* obp, off64_t off_start, off64_t off_end,
...
int memobj_pmem_walk_mm(int flags, struct mm_map* mm, int (*func)(OBJECT*, off64_t, struct pa_quantum*, unsigned, void*), void* d)
{
    return memobj_pmem_walk(flags, mm->obj_ref->obp, mm->offset, mm->offset + (mm->end - mm->start), func, d);
}
