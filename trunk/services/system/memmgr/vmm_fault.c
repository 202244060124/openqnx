...
int vmm_fault(struct fault_info* info)
{
    ...
    if (r == 0) {
        ...
        // Forced bad page
        info->sigcode = MAKE_SIGCODE(SIGBUS, BUS_OBJERR, FLTPAGE) | (info->sigcode & SIGCODE_FLAGS_MASK);
        r = -1;
    }
...
#define MAX_FAULT_IN ((16 * __PAGESIZE) - 1)
    ...
    r = memory_reference(&mm, vaddr, vend, (sigcode & SIGCODE_STORE) ? (MR_WRITE | MR_TRUNC) : MR_TRUNC, &ms);
    lcp = prp->lcp;
    if ((lcp != NULL) && (r != EOK)) {
        lcp->fault_errno = r;
    }
