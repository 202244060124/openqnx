...
static int io_mmap(struct _msg_info* info, int fd, resmgr_context_t* ctp, mem_map_t* mmapp, unsigned prot)
{
    io_mmap_t msg;
...
again:
    ...
    if ((obp->fdmem.ino && obp->fdmem.ino != sbuf.st_ino) || (obp->fdmem.dev && obp->fdmem.dev != sbuf.st_dev)) {
...
{
    if (!(atomic_set_value(&fdmem_cleanup, MMAPFD_CLEANUP_QUEUED) & (MMAPFD_CLEANUP_QUEUED | MMAPFD_CLEANUP_INPROGRESS))) {
...
#define TIME_INFINITY (~(time_t)0)
    ...
    if (atomic_set_value(&fdmem_cleanup, MMAPFD_CLEANUP_INPROGRESS) & MMAPFD_CLEANUP_INPROGRESS) {
