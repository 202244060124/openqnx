...
static struct pa_restrict restrict_4G[] = {{restrict_4G + 1, NULL, CPU_SYSTEM_PADDR_START, CPU_SYSTEM_PADDR_END},
...
int cpu_vmm_mcreate(PROCESS* prp)
{
    ...
    if (prp->pid == PROCMGR_PID) {
        ...
        r = vmm_mmap(NULL, 0, pdir_size, PROT_READ | PROT_WRITE, MAP_PHYS, NULL, paddr, 0, 0, NOFD, &vaddr, &size, mpid);
        if (r != EOK) {
            return r;
        }
        if (pae_enabled) {
...
#define PTE_CHECK_BITS                                                                                                       \
    (X86_PTE_PRESENT | X86_PTE_WRITE | X86_PTE_USER | X86_PTE_WT | X86_PTE_CD | X86_PTE_PAT | X86_PTE_GLOBAL | X86_PTE_USER1 \
     | X86_PTE_USER2 | X86_PTE_USER3 | X86_PTE_NX | PTE_PADDR_BITS)
...
int cpu_pte_merge(struct mm_pte_manipulate* data)
{
    ...
    while (check < data->split_end) {
        ...
        if ((pde & X86_PTE_PRESENT) && !(pde & X86_PDE_PS)) {
            ...
            if ((adp == NULL) || (check > CPU_USER_VADDR_END) || (adp->cpu.ptroot_paddr == rdpgdir())) {
...
#define PMF_CR0_FLIPPED 0x0040
    ...
    for (;;) {
        ...
        if (op & (PTE_OP_MAP | PTE_OP_BAD)) {
            ...
            // PTE_OP_PROT
            pte = (orig_pte & (PTE_PADDR_BITS | X86_PTE_ACCESSED | X86_PTE_DIRTY)) | bits;
        } else {
...
unsigned cpu_vmm_vaddrinfo(PROCESS* prp, uintptr_t vaddr, paddr_t* paddrp, size_t* lenp)
{
    ...
    if ((prp == NULL) || (vaddr > CPU_USER_VADDR_END) || (prp->memory->cpu.ptroot_paddr == rdpgdir())) {
        // We can use the currently active page table
