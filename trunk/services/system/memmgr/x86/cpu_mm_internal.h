...
#define CPU_1TO1_IS_VADDR(v) \
    ((__cpu_flags & X86_CPU_PSE) && ((v) >= CPU_1TO1_VADDR_BIAS) && ((v) <= (CPU_1TO1_VADDR_BIAS + CPU_SYSTEM_PADDR_END)))
...
#pragma aux _zero_dwords = "cld"       \
                           "mov eax,0" \
                           "rep stosd" parm[edi][ecx] modify exact[eax ecx edi];
