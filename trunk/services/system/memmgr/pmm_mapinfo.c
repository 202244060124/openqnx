...
static int pmm_check(uintptr_t vaddr, struct mem_phys_entry* mem, unsigned memaddr, struct mem_phys_entry** closest, unsigned* addr)
{
    if (vaddr >= memaddr) {
...
size_t pmm_mapinfo(PROCESS* prp, uintptr_t vaddr, struct _procfs_map_info* mip, struct _procfs_debug_info* mdp, size_t dbginfo_pathlen,
                   OBJECT** obpp, int* fdp, size_t* contigp)
{
