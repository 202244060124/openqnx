...
#define COLOUR_ADJUST_VA(addr, addr_colour) ((addr) + (((addr_colour) - (addr)) & colour_mask_shifted))
...
#define COLOUR_ADJUST_PA(paddr, paddr_colour)                                                                          \
    ((((paddr_colour) >> ADDR_OFFSET_BITS) == PAQ_COLOUR_NONE) || ((COLOUR_PA((paddr)) == (COLOUR_PA(paddr_colour))))) \
        ? (paddr)                                                                                                      \
        : ((COLOUR_ADJUST_VA((paddr), (paddr_colour))) - ((colour_mask + 1) << ADDR_OFFSET_BITS))
...
struct pa_quantum* pa_alloc(paddr_t size, paddr_t align, unsigned colour, unsigned flags, unsigned* status, struct pa_restrict* restriction,
                            paddr_t resv_size);
...
#define pa_free_paddr(p, s, r) pa_free(pa_paddr_to_quantum(p), LEN_TO_NQUANTUM(s), r)
