...
peer_restart:
    ...
    switch (msg->type) {
        case _MEM_PEER:
            ...
            memmove(msg, (uint8_t*)msg + msg->peer.i.peer_msg_len, (ctp->info.msglen - msg->peer.i.peer_msg_len));
            switch (msg->type) {
                case _MEM_MAP:
                case _MEM_CTRL:
                    src = prp->cred;
                    dst = peer->cred;
                    if (!(src->info.euid == 0 || src->info.ruid == dst->info.ruid || src->info.ruid == dst->info.suid
