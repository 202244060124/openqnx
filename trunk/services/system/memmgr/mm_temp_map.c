...
int pte_temp_map(ADDRESS* adp, uintptr_t user_va, struct pa_quantum* pq, struct mm_map* mm, size_t len, int (*func)(void*, size_t, void*),
                 void* d)
{
    paddr_t paddr;
    uintptr_t end;
    ...
    if (CPU_1TO1_IS_PADDR(paddr + len)
        ...
        && (((obp->hdr.type != OBJECT_MEM_SHARED) && (obp->hdr.type != OBJECT_MEM_TYPED)) || !(obp->mem.mm.flags & MM_SHMEM_SPECIAL))) {
...
#define MAX_TEMP_MAPPING (64 * 1024)
     ...
     */
    if (piece <= adp->tmap_size && ((adp->tmap_base >= (user_va + len)) || ((adp->tmap_base + adp->tmap_size) <= user_va))) {
        ...
        for (;;) {
            ...
            // to fiddle things.
            tmap_start = map_find_va(&adp->map, user_va & ~colour_mask_shifted, piece, colour_mask_shifted, 0);
    ...
    for (;;) {
        ...
        r = pte_map(adp, vaddr, end, mm->mmap_flags | (PROT_READ | PROT_WRITE), obp, paddr, PTE_OP_TEMP);
        if (r != EOK)
            break;
        if (vaddr < use_start)
            use_start = vaddr;
