...
static int (*const fs_check[])(const resmgr_io_funcs_t*, mem_map_t*, void*, OBJECT**) = {devmem_check, devzero_check, imagefs_check};
...
check_again:
    ...
    for (;;) {
        int lock_status;
        status = memmgr.mmap(prp, msg->i.addr, msg->i.len, msg->i.prot, msg->i.flags, object, msg->i.offset, msg->i.align, msg->i.preload,
                             msg->i.fd, &addr, &size, mpart_id);
