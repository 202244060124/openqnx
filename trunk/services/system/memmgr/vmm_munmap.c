...
static int clean_pmem(OBJECT* obp, off64_t off, struct pa_quantum* pq, unsigned num, void* d)
{
    ...
    if (pq->blk == PAQ_BLK_FAKE) {
        ...
        if (pq->flags & PAQ_FLAG_MODIFIED) {
            ...
            MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr), sync_off, sync_off + ((num << QUANTUM_BITS) - 1), data->prp,
                            (void*)(mm->start + (uintptr_t)(off - mm->offset)));
        }
        ...
        do {
            ...
            if (pq_flags & PAQ_FLAG_HAS_SYNC) {
                ...
                sync_off = PADDR_TO_SYNC_OFF(paddr);
                MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr), sync_off, sync_off + (QUANTUM_SIZE - 1), prp, (void*)vaddr);
            }
            vaddr += QUANTUM_SIZE;
            paddr += QUANTUM_SIZE;
            ++pq;
...
int ms_unmap(ADDRESS* adp, struct map_set* ms, int unmap_flags)
{
    ...
    for (mm = ms->first; mm != end_mm; mm = mm->next) {
        ...
        if (or != NULL) {
            ...
            if ((data.flags & PAQ_FLAG_MODIFIED) && !(mm->mmap_flags & MAP_NOSYNCFILE) && (obp->hdr.type == OBJECT_MEM_FD)
            ...
            if (obp->hdr.type != OBJECT_MEM_ANON) {
                if (((mm->mmap_flags & (MAP_LAZY | MAP_TYPE)) == MAP_PRIVATE) && !(data.unmap_flags & UNMAP_PRIVATIZE)) {
                    OBJECT* anmem_obp;
                    off64_t anmem_off;
                    size_t anmem_size;
                    int anmem_lock;
