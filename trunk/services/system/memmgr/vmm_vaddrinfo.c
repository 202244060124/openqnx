...
unsigned vmm_vaddrinfo(PROCESS* prp, uintptr_t vaddr, paddr_t* paddrp, size_t* lenp, unsigned flags)
{
    ADDRESS* adp;
    struct map_set ms;
    struct mm_map* mm;
        ...
        if (r == EOK) {
            ...
            if (mm != NULL) {
                if (mm->obj_ref != NULL) {
                    ...
                    obp = mm->obj_ref->obp;
                    memobj_lock(obp);
                    memobj_pmem_walk(MPW_PHYS | MPW_SYSRAM, obp, data.off, data.off, get_paddr, &data);
                    memobj_unlock(obp);
