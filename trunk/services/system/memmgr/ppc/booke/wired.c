...
static void wire_aspace(PROCESS* aspaceprp, PROCESS** paspaceprp)
{
    vmm_aspace(aspaceprp, paspaceprp);
    wire_sync(aspaceprp->memory);
}

void wire_check(struct mm_pte_manipulate* data)
{
    struct wire_entry* we;
    unsigned i;
    uintptr_t vaddr;
    ...
    for (;;) {
        ...
        if ((vaddr & mask) == (we->tlb.epn & mask)) {
            if (!we->perm
                && ((we->tlb.rpn != new.rpn) || (we->tlb.attr != new.attr) || (we->tlb.access != new.access) || (we->tlb.size != new.size)
                    || (we->tlb.ts != new.ts) || (we->tlb.v != new.v))) {
...
static int wire_mcreate(PROCESS* prp)
{
    struct wire_entry* we;
    unsigned i;
    int r;
