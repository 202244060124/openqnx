...
void fam_pte_flush_all()
{
    tlbop.flush_all();
    SMP_FLUSH_TLB(); // send IPI because tlbwe is not broadcast
}

void fam_pte_mapping_add(uintptr_t vaddr, paddr_t paddr, unsigned prot, unsigned flags)
{
    ppcbke_tlb_t new;
    ...
    new.access = 0;
    if (prot & PROT_READ)
        new.access |= PPCBKE_TLB_ACCESS_SR | PPCBKE_TLB_ACCESS_UR;
    if (prot & PROT_WRITE)
        new.access |= PPCBKE_TLB_ACCESS_SW | PPCBKE_TLB_ACCESS_UW;
    if (prot & PROT_EXEC)
        new.access |= PPCBKE_TLB_ACCESS_SX | PPCBKE_TLB_ACCESS_UX;
...
void fam_pte_mapping_del(ADDRESS* adp, uintptr_t vaddr, uint32_t size)
{
    while (size > 0) {
        /*
...
void fam_pte_asid_alloc(ADDRESS* adp)
{
...
void fam_pte_asid_release(ADDRESS* adp)
{
    int flush;
#ifdef VARIANT_smp
...
int cpu_pte_split(uintptr_t vaddr, struct mm_pte_manipulate* data)
{
    ...
    if ((data->op & PTE_OP_UNMAP) || (data->shmem_flags & SHMCTL_HIGHUSAGE) || (vps_state == VPS_AUTO)) {
        ...
        if (pde != NULL) {
            ...
            if (PTE_PRESENT(ptep)) {
                ...
                if ((check < vaddr) || ((vaddr == data->start) && (check_end > data->end))) {
                    ...
                    if (check_end > data->split_end)
                        data->split_end = check_end;
                    new_flags = (ptep->flags & ~PPCBKE_PTE_PGSZ_MASK) | (PPCBKE_TLB_SIZE_4K << PPCBKE_PTE_PGSZ_SHIFT);
    ...
    if (!(data->op & PTE_OP_TEMP) && (data->op & PTE_OP_UNMAP) && (data->shmem_flags & SHMCTL_HIGHUSAGE)) {
    ...
    if (!(data->op & PTE_OP_MERGESTARTED)) {
        ...
        for (;;) {
            ...
            if (chk_ptep != NULL) {
                ...
                if (PTE_PERMS(ptep) != PTE_PERMS(chk_ptep))
                    break;
                if ((PTE_PADDR(ptep) + pgsz) != PTE_PADDR(chk_ptep))
                    break;
                data->start = chk_vaddr;
            }
            if (chk_vaddr == merge_start)
                break;
            chk_ptep = ptep;
    ...
    do {
        ...
        for (;;) {
            ...
            if (chk_ptep == NULL) {
                ...
                if (PTE_PERMS(ptep) != PTE_PERMS(chk_ptep))
                    break;
                if ((PTE_PADDR(chk_ptep) + PTE_PGSIZE(chk_ptep)) != PTE_PADDR(ptep))
                    break;
            }
            run_last = data->start;
            data->start += PTE_PGSIZE(ptep);
            if (data->start == 0)
                data->start = ~(uintptr_t)0;
            chk_ptep = ptep;
        }
        ...
        if (chk_ptep != NULL) {
            ...
            while (run_start < run_last) {
                ...
                do {
                    pgsz = *pgszp++;
                } while ((pgsz > ((run_last - run_start) + __PAGESIZE)) || ((run_start & (pgsz - 1)) != 0) || ((paddr & (pgsz - 1)) != 0));
...
int cpu_pte_manipulate(struct mm_pte_manipulate* data)
{
    ...
    if (data->op & PTE_OP_BAD) {
        ...
        if (data->prot & PROT_EXEC)
            bits |= PPCBKE_PTE_ACCESS_X;
        if (data->prot & PROT_WRITE)
            bits |= PPCBKE_PTE_ACCESS_W;
        if (data->prot & PROT_READ)
            bits |= PPCBKE_PTE_ACCESS_R;
    ...
    for (;;) {
        ...
        if (data->op & (PTE_OP_MAP | PTE_OP_BAD)) {
            ...
            ptep->flags = (bits & ~PPCBKE_PTE_PGSZ_MASK) | (ptep->flags & PPCBKE_PTE_PGSZ_MASK);
        } else {
