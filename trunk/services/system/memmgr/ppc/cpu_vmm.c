...
int cpu_vmm_fault(struct fault_info* info)
{
    ...
    if (!(flags & VM_FAULT_WRITE)) {
        if (vaddr >= VM_SYSPAGE_ADDR && vaddr < (VM_SYSPAGE_ADDR + __PAGESIZE)) {
            fam_pte_mapping_add(VM_SYSPAGE_ADDR, (uintptr_t)_syspage_ptr, PROT_EXEC | PROT_READ, flags | VM_FAULT_GLOBAL);
            return 1;
        }
        if (!(flags & VM_FAULT_INSTR) && vaddr >= VM_CPUPAGE_ADDR && vaddr < (VM_CPUPAGE_ADDR + __PAGESIZE)) {
            ...
            fam_pte_mapping_add(VM_CPUPAGE_ADDR, (uintptr_t)pp->kern_cpupageptr + (RUNCPU * pp->cpupage_spacing), PROT_READ,
                                flags | VM_FAULT_GLOBAL);
            return 1;
        }
    }
    ...
    if (vaddr >= VM_MSG_XFER_START && vaddr < VM_MSG_XFER_END && (info->sigcode & SIGCODE_INXFER)) {
        ...
        if (pde != NULL) {
            ...
            if (PTE_PRESENT(ptep)) {
                if (PTE_READABLE(ptep)) {
                    prot = PROT_READ;
                    if (PTE_WRITEABLE(ptep))
                        prot |= PROT_WRITE;
                    if (!PTE_CACHEABLE(ptep))
                        prot |= PROT_NOCACHE;
                    if ((prot & PROT_WRITE) || (!(flags & VM_FAULT_WRITE))) {
                        ...
                        if (xfer_lastaddr[xfer_rotor]) {
                            fam_pte_mapping_del(adp, xfer_lastaddr[xfer_rotor], __PAGESIZE);
                        }
                        if (xfer_rotor > 1)
                            crash();
                        xfer_lastaddr[xfer_rotor] = ADDR_PAGE(vaddr);
                        paddr = PTE_PADDR(ptep) + (real_addr & ((PTE_PGSIZE(ptep) - 1) & ~(__PAGESIZE - 1)));
...
int cpu_vmm_mcreate(PROCESS* prp)
{
    ...
    adp = prp->memory;
    if (vmm_mmap(NULL, 0, L1TBL_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_PHYS, 0, 0, __PAGESIZE, 0, NOFD, &vaddr, &size,
                 mpid)
        != EOK) {
        return ENOMEM;
    }
