...
static ptrdiff_t vmm_map_addr_old(PROCESS* actprp, PROCESS* prp, void* addr, size_t size, void** plo, void** phi, int flags)
{
    uint8_t* base;
    ADDRESS* adp;
...
static int vmm_map_addr_general(PROCESS* actprp, PROCESS* prp, IOV** piov, int* pparts, int* poff, IOV* niov, int* pnparts, unsigned flags)
{
    void *hi, *lo;
    IOV *iov, *iov_o;
    ...
    while (base <= last) {
        if (xfer_memprobe((void*)(base + diff)) != 0)
            return -1;
        base += __PAGESIZE;
    }
    ...
    while (nparts < nparts_max) {
        ...
        if (len) {
            ...
            if (base < (uintptr_t)lo || last > (uintptr_t)hi) {
                ...
                diff = vmm_map_addr_old(actprp, prp, (void*)base, len, &lo, &hi, flags);
                if (diff == -1) {
                    return -1;
                }
            }
...
#define TRANSLATE_IOV_ADDR(iov, iov_phy)                                                                                        \
    {                                                                                                                           \
        pte_t* pde;                                                                                                             \
        pte_t* ptep;                                                                                                            \
        if ((pde = PDE_ADDR(pgdir[L1PAGEIDX(iov)])) == 0 || !PTE_PRESENT(ptep = &pde[L2PAGEIDX(iov)]) || !PTE_READABLE(ptep)) { \
            return -1;                                                                                                          \
        }                                                                                                                       \
        ...
        if (xfer_memprobe(iov_phy) != 0)                                                                                        \
            return -1;                                                                                                          \
    }

int vmm_map_xfer(PROCESS* actprp, PROCESS* prp, IOV** piov, int* pparts, int* poff, IOV* niov, int* pnparts, unsigned flags)
{
    unsigned len;
    IOV *iov_phy, *iov;
    IOV* orig_niov = niov;
    ...
    while (off >= (size = GETIOVLEN(iov_phy))) {
        ...
        ++iov_phy;
        addr = 0;
        if ((ADDR_OFFSET((uintptr_t)iov_phy + sizeof(_Int32t)) <= sizeof(_Int32t)) && !(flags & MAPADDR_FLAGS_IOVKERNEL)) {
    ...
    do {
        ...
        if (addr < VM_USER_SPACE_BOUNDRY) {
            ...
            do {
                ...
                if (!PTE_PRESENT(pteop) || (flags & MAPADDR_FLAGS_WRITE) ? !PTE_WRITEABLE(pteop) : !PTE_READABLE(pteop)) {
                    /*
                ...
                for (;;) {
                    ...
                    if (len >= size)
                        break;
                    next_addr = addr + len;
                    if (L1PAGEIDX(next_addr) != l1pidx)
                        break;
                    ptep = &pde[L2PAGEIDX(next_addr)];
                    // check physical mem continuty
                    if ((PTE_PERMS(ptep)) != PTE_PERMS(pteop))
                        break;
                    if ((PTE_PADDR(pteop) + pg_size) != PTE_PADDR(ptep))
                        break;
        ...
        do {
            if (--parts == 0)
                break;
            iov_phy++;
            if ((ADDR_OFFSET((unsigned)iov_phy + sizeof(_Int32t)) <= sizeof(_Int32t)) && !(flags & MAPADDR_FLAGS_IOVKERNEL)) {
