...
size_t vmm_mapinfo(PROCESS* prp, uintptr_t vaddr, struct _procfs_map_info* mip, struct _procfs_debug_info* mdp, size_t dbginfo_pathlen,
                   OBJECT** obpp, int* fdp, size_t* contigp)
{
    ...
    if (or != NULL) {
        ...
        if ((contigp != NULL) || (mip != NULL)) {
            ...
            memobj_pmem_walk(MPW_SYSRAM | MPW_PHYS | MPW_HOLES, obp, info.offset, info.offset + info.size - 1, pmem_info, &data);
