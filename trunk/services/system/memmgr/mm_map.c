...
need_more:
    ...
    if (memmgr.mmap(0, 0, MAP_BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, 0, 0, 0, 0, NOFD, &addr, &size,
...
static struct mm_map_internal* rdecl split_one(struct mm_map_head* mh, struct mm_map_internal* mm, uintptr_t split)
{
...
int map_isolate(struct map_set* ms, struct mm_map_head* mh, uintptr_t start, size_t size, int flags)
{
    ...
    if (prev == NULL || (mm = (struct mm_map_internal*)prev->map.next) == NULL || mm->map.start > start) {
...
uintptr_t map_find_va(struct mm_map_head* mh, uintptr_t va, uintptr_t size, uintptr_t mask, unsigned flags)
{
...
int map_create(struct map_set* ms, struct map_set* repl, struct mm_map_head* mh, uintptr_t va, uintptr_t size, uintptr_t mask,
               unsigned flags)
{
    struct mm_map_internal* new;
    struct mm_map* mm;
...
int map_remove(struct map_set* ms)
{
    ...
    rover = ms->head->rover;
    if (rover != NULL && (rover_start = rover->start) >= ms->first->start && rover_start <= ms->last->start) {
