...
#define SET_TAIL(_qp, _runsize) \
    if ((_qp)->run > 1)         \
    ((_qp)[(_qp)->run - 1].run = (_runsize))
...
static allocator_accessfncs_t afncs = {STRUCT_FLD(reserve) pa_reserve, STRUCT_FLD(unreserve) pa_unreserve,
                                       STRUCT_FLD(size_info) pa_size_info, STRUCT_FLD(resv_adjust) pa_resv_adjust};
static memclass_attr_t sysram_attr = {
    STRUCT_FLD(size) 0, // filled in in pa_init()
    STRUCT_FLD(limits){STRUCT_FLD(alloc){STRUCT_FLD(size){STRUCT_FLD(min) __PAGESIZE, STRUCT_FLD(max) memsize_t_INFINITY}}},
};
    ...
    while (chk < &bh->quanta[bh->num_quanta]) {
        ...
        for (i = 1; i < chk->run; ++i) {
            if (chk[i].run > NON_HEAD_QUANTUM) {
                ...
                // kprintf("RUN blk #%d %d %d [%d] %d %d (%d)\n",chk->blk,chk[i-2].run,chk[i-1].run,chk[i].run,chk[i+1].run,chk[i+2].run,
                // chk[chk[i].run].run);
                crash();
            }
        }
...
#error code does not support CPU_SYSTEM_PADDR_START != 0
    ...
    return cpu_early_paddr_to_vaddr(sl->list[max].start, ROUNDUP(size, QUANTUM_SIZE), &sl->list[max].end);
}
...
static unsigned init_block(struct block_head** bhp, unsigned blk_idx, struct sysram_entry* se, struct pa_quantum* quanta)
{
    unsigned num;
    struct pa_quantum* pq;
    unsigned i;
...
static unsigned pa_carve(struct block_head* bhp, struct pa_quantum* head, struct pa_quantum* start, size_t len, unsigned* colour)
{
...
static void kerext_pa_alloc(void* d)
{
    ...
    for (curr_res = kap->restriction; curr_res; curr_res = curr_res->next) {
        ...
        pgsz = pgszlist;
        for (curr_size = (flags & PAA_FLAG_CONTIG) ? size_left : pa_largest_pow2(size_left); curr_size >= QUANTUM_SIZE; curr_size >>= 1) {
            ...
            for (;;) {
                ...
                */
                ...
                /* restriction ends outside (or is NULL b/c its way out),
                   take up end slack */
                if ((mod_rq_end == NULL) || (mod_rq_end > qblk_end) || (mod_rq_end->blk != qblk_end->blk)) {
                    mod_rq_end = qblk_end;
                }
                /* restriction starts outside the blk, but ends inside,
                   tighten the start */
                if ((mod_rq_start == NULL) || (mod_rq_start < qblk_start) || (mod_rq_start->blk != qblk_start->blk)) {
                ...
                */
                ...
                for (;;) {                     /* manually control the loop */
                    ...
                    if ((qp->run < LEN_TO_NQUANTUM(curr_size)) || ((qp_end < mod_rq_start) || (qp > mod_rq_end))) {
                        qp = qp->u.flink.next;
...
struct pa_quantum* pa_alloc(paddr_t size, paddr_t align, unsigned colour, unsigned flags, unsigned* status_p,
                            struct pa_restrict* restriction, paddr_t resv_size)
{
...
#ifdef TRACKPMEM
    {
        ...
        for (p = data.alloc; p; p = p->u.flink.next) {
            if (p == data.alloc)
                kprintf("%s ALLOC 0x%x for %d [free=0x%x]\n", (p == data.alloc) ? "PMEM" : "    ", (uintptr_t)p, p->run,
                        (unsigned)mem_free_size);
        }
    }
#endif
...
static void _pa_free(struct pa_quantum* paq, unsigned num, paddr_t resv_size)
{
    ...
    if (!(run_start->flags & PAQ_FLAG_INUSE)) {
        kprintf("paq %p paq_head %p, paq->run %d, runhead r/f/bl = %d/%08x/%d\n", paq, run_start, paq->run, run_start->run,
                run_start->flags, run_start[run_start->run - 1].run);
        crash();
    }
#endif
    ...
    } else if ((next < &bh->quanta[bh->num_quanta]) && !(next->flags & PAQ_FLAG_INUSE)) {
...
static int pmem_add_cleanup(message_context_t* ctp, int code, unsigned flags, void* handle)
{
    ...
    for (;;) {
        ...
        if (timerid < 0)
            return EOK;
        if ((SIGEV_GET_TYPE(&info.event) == SIGEV_PULSE) && (info.event.sigev_code == add_code) && (info.event.sigev_value.sival_ptr == bhp))
            break;
        timerid = timerid + 1;
    }
...
memsize_t pa_free_size(void)
{
    ...
    if (total_free != (mem_free_size + mem_reserved_size)) {
        kprintf("total_free (%P) != mem_free_size + mem_reserved_size (%P + %P)\n", (paddr_t)total_free, (paddr_t)mem_free_size,
                (paddr_t)mem_reserved_size);
        //                crash();
    }
#endif /* NDEBUG */
