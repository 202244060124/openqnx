...
#define PTE_PGSIZE_TO_ATTR(size) \
    ((size == 4096) ? SH_CCN_PTEL_SZ0 : (size == MEG(1)) ? (SH_CCN_PTEL_SZ0 | SH_CCN_PTEL_SZ1) : SH_CCN_PTEL_SZ1)
...
void sh4_update_tlb(uintptr_t vaddr, uint32_t ptel)
{
    unsigned intr, pteh;
 ...
 */
static struct sh4_tlb* perm_map_check(uintptr_t badva, unsigned write_fault, unsigned load_tlb)
{
    int i;
    uintptr_t mask;
    struct sh4_tlb* perm;

    if (!num_permanent_tlb)
        return NULL;
    ...
    for (i = num_permanent_tlb - 1; i >= 0; --i) {
        ...
        if ((perm->vaddr & mask) == (badva & mask)) {
            ...
            if (write_fault && !(perm->data & SH_CCN_PTEL_PR(1)))
                return 0;
...
static void sh4_gettlb(struct sh4_tlb* tlb, unsigned i)
{
    unsigned ptr;
    uint32_t value;
...
static void* alloc_pgentry(PROCESS* prp)
{
...
int cpu_vmm_mcreate(PROCESS* prp)
{
    ...
    r = pte_map(adp, VM_TIMERPAGE_ADDR, VM_TIMERPAGE_ADDR + __PAGESIZE - 1, PROT_READ | PROT_NOCACHE, NULL, paddr, 0);
    if (r != EOK)
        goto fail2;

    // For backwards compatability
    paddr = P4_TO_A7(SH_MMR_TMU_TOCR);
    r = pte_map(adp, 0x7fff8000, 0x7fff8000 + __PAGESIZE - 1, PROT_READ | PROT_NOCACHE, NULL, paddr, 0);
    if (r != EOK)
        goto fail2;
    ...
    if (!(data->op & PTE_OP_MERGESTARTED)) {
        ...
        for (;;) {
            ...
            if (chk_ptep != NULL) {
                ...
                pte = *ptep;
                chk_pte = *chk_ptep;
                if (PTE_PERMS(pte) != PTE_PERMS(chk_pte))
                    break;
                if ((REAL_ADDR(pte) + pgsz) != REAL_ADDR(chk_pte))
                    break;
    ...
    do {
        ...
        for (;;) {
            ...
            if (chk_ptep == NULL) {
                ...
                if (PTE_PERMS(pte) != PTE_PERMS(chk_pte))
                    break;
                if ((REAL_ADDR(chk_pte) + SH_PTE_PGSIZE(chk_pte)) != REAL_ADDR(pte))
                    break;
            }
            run_last = data->start;
...
unsigned cpu_vmm_vaddrinfo(PROCESS* prp, uintptr_t vaddr, paddr_t* paddrp, size_t* lenp)
{
    ...
    *paddrp = (pte & mask) | (vaddr & ~mask);
    ...
    if (IS_PAGEREADABLE(pte))
        prot |= PROT_READ | PROT_EXEC;
    if (IS_PAGEWRITEABLE(pte))
        prot |= PROT_WRITE;
    return prot;
}
