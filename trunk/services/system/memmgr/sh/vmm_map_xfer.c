 ...
 */
static inline unsigned map_slot0(PROCESS* prp, struct xfer_map* map, unsigned addr, unsigned end)
{
 ...
 */
static inline unsigned map_slot1(PROCESS* prp, struct xfer_map* map, unsigned addr)
{
    ADDRESS* dadp = prp->memory;
    ADDRESS* sadp = map->prp->memory;
