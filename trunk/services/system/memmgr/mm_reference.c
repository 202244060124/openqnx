...
static int rdecl condition_cache(struct mm_map* mm, struct pa_quantum* pq, unsigned num, struct data* data)
{
    int r;
...
static int pq_init_run(void* dst, size_t len, void* d)
{
    ...
    switch (obp->hdr.type) {
        ...
        case OBJECT_MEM_FD:
            ...
            got = proc_read(obp->fdmem.fd, dst, len - last_page_bss, mm->offset + (data->va - mm->start));
...
#define INIT_REQUIRED(mmf, pq) (!((pq)->flags & PAQ_FLAG_INITIALIZED) && (((pq)->flags & PAQ_FLAG_INIT_REQUIRED) || !((mmf)&MAP_NOINIT)))
...
static int pq_map(OBJECT* obp, off64_t off, struct pa_quantum* pq, unsigned num, void* d)
{
    ...
    if (pq->blk == PAQ_BLK_FAKE) {
        ...
        if (!(mm->extra_flags & EXTRA_FLAG_CACHE_CLEANED)) {
            ...
            // Aaarrrggghhh!
            if ((((mm->end - mm->start) + 1) != NQUANTUM_TO_LEN(num)) && (obp->hdr.type == OBJECT_MEM_ANON)) {
                crash();
            }
...
int memory_reference(struct mm_map** mmp, uintptr_t start, uintptr_t end, unsigned flags, struct map_set* ms)
{
    OBJECT* obp;
    struct mm_map* mm;
    struct mm_object_ref* or ;
    ...
    if (mm->mmap_flags & PROT_WRITE) {
        if (!CPU_FAULT_ON_WRITE_WORKS || (or->adp->flags & MM_ASFLAG_ISR_LOCK)) {
            ...
            if (((mm->mmap_flags & MAP_TYPE) == MAP_PRIVATE) && (obp->hdr.type != OBJECT_MEM_ANON) && (flags & MR_NOINIT)) {
                return EOK;
            }
        }
    }
