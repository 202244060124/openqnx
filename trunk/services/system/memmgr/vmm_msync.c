...
static int pte_off(struct mm_object_ref* or, struct mm_map* mm, void* d)
{
    ...
    if (!(adp->flags & MM_ASFLAG_ISR_LOCK)) {
        ...
        if (flags & data->bits) {
            ...
            if ((start >= sync_start) && (start <= sync_end) || (end >= sync_start) && (end <= sync_end)) {
                // In range
                if (start < sync_start)
                    start = sync_start;
                if (end > sync_end)
...
static void one_cache(struct cache_data* cache, uintptr_t start, uintptr_t next)
{
    if (cache->start == VA_INVALID) {
        ...
        CPU_CACHE_CONTROL(cache->as, (void*)cache->start, cache->next - cache->start, cache->flags);
        cache->start = start;
    }
    cache->next = next;
}
    ...
    if (!(flags & (MS_INVALIDATE_ICACHE | MS_CACHE_ONLY))) {
        for (mm = ms.first; mm != ms.last->next; mm = mm->next) {
            ...
            if (or != NULL) {
                ...
                if ((flags & MS_INVALIDATE) || ((flags & (MS_SYNC | MS_ASYNC)) && !(mm->mmap_flags & MAP_NOSYNCFILE))) {
                    unsigned has_lock = memobj_cond_lock(obp);
                    if ((obp->hdr.type == OBJECT_MEM_FD) && ((obp->mem.mm.flags & MM_FDMEM_NEEDSSYNC) || (flags & MS_INVALIDATE))) {
                        /*
