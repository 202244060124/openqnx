...
static int match_name(struct asinfo_entry* base_as, struct asinfo_entry* as, const char* name, unsigned name_len)
{
    ...
    return match_name(base_as, (struct asinfo_entry*)((uint8_t*)base_as + as->owner), name, (name_len - len) - 1);
}
...
struct pa_quantum* tymem_rdb_alloc(OBJECT* obp, size_t size, unsigned flags)
{
    ...
    restart:
        ...
        for (;;) {
            ...
            if ((e->data.info.size.reserved.free < resv_size) || (e->data.info.size.unreserved.free < (size - resv_size))) {
                goto fail1;
            }
