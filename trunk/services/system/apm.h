...
typedef struct rsrcmgr_mempart_fnctbl_s {
    ...
    void (*mclass_event_handler)(struct apmmgr_attr_s* attr, memclass_evttype_t evtType, memclass_sizeinfo_t* cur,
 ...
 */
#define MEMPART_CHK_and_INCR(mp, inc, r) ((!MEMPART_INSTALLED()) ? EOK : mempart_fnctbl->chk_and_incr((mp), (inc), (r)))
...
#define MEMPART_UNDO_INCR(mp, inc, r)                \
    if (MEMPART_INSTALLED()) {                       \
        mempart_fnctbl->undo_incr((mp), (inc), (r)); \
    }
 ...
 */
#define MEMPART_DECR(mp, free_size)           ((!MEMPART_INSTALLED()) ? 0 : mempart_fnctbl->decr((mp), (free_size)))
 ...
 */
#define MEMPART_INCR(mp, req_size)            ((!MEMPART_INSTALLED()) ? EOK : mempart_fnctbl->incr((mp), (req_size)))
 ...
 */
#define MEMPART_GETLIST(p, mplist, n, f, c)   mempart_fnctbl->get_mempartlist((p), (mplist), (n), (f), (c))
 ...
 */
#define MEMPART_VALIDATE_ID(mpid)             (MEMPART_INSTALLED() ? mempart_validate_id((mpid)) : EINVAL)
 ...
 *
 */
#define MEMPART_VALIDATE_ASSOCIATION(mpid, c) (MEMPART_INSTALLED() ? mempart_fnctbl->validate_association((mpid), (c)) : EINVAL)
...
#define MEMSIZE_OFLOW(s) (((s) != memsize_t_INFINITY) && (((s)&0xFFFFFFFF00000000ULL) != 0ULL))
