...
#define OVERLAP(s1, e1, s2, e2)    (((s1) <= (s2) && (s2) <= (e1)) || ((s2) <= (s1) && (s1) <= (e2)))
#define CONTAINS(s1, e1, s2, e2)   (((s1) <= (s2) && (s2) <= (e1)) && ((s1) <= (e2) && (e2) <= (e1)))
#define CONTAINSNE(s1, e1, s2, e2) (((s1) < (s2) && (s2) < (e1)) && ((s1) < (e2) && (e2) < (e1)))
...
static void add_element(struct _alloc_array* array, rsrc_alloc_t* alloc)
{
    if (array->index >= array->count) {
        array->array =
            (rsrc_alloc_t*)_srealloc(array->array, array->count * sizeof(rsrc_alloc_t), (array->count + 10) * sizeof(rsrc_alloc_t));
        array->count += 10;
    }
...
static int add_range(struct _alloc_array* array, rsrc_alloc_t* alloc)
{
    ...
    for (i = 0; i < insstart; i++) {
        for (j = insstart; j < array->index; j++) {
            ...
            if ((array->array[i].flags & RSRCDBMGR_TYPE_MASK) != (array->array[j].flags & RSRCDBMGR_TYPE_MASK)) {
                continue;
            }

            // Check for overlaping start/end
            if (!(OVERLAP(array->array[i].start, array->array[i].end, array->array[j].start, array->array[j].end))) {
                continue;
            }

            // If the new block is completely contained, nuke it
            if (CONTAINS(array->array[i].start, array->array[i].end, array->array[j].start, array->array[j].end)) {
                array->index--;
            ...
            if (CONTAINSNE(array->array[j].start, array->array[j].end, array->array[i].start, array->array[i].end)) {
    ...
    for (i = insstart; i < insend; i++) {
        ...
        if (lastindex >= 0) {
            memmove(&array->array[lastindex + 1], &array->array[lastindex], (i - lastindex) * sizeof(extra));
