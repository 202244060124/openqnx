...
schedpart_fnctbl_t proxy_schedpart_fnctbl = {
    STRUCT_FLD(associate) schedpart_proc_associate, STRUCT_FLD(disassociate) schedpart_proc_disassociate,
    STRUCT_FLD(get_schedpart) schedpart_nodeget,    STRUCT_FLD(get_schedpartlist) schedpart_getlist,
    STRUCT_FLD(validate_association) NULL,          STRUCT_FLD(rsrcmgr_attach) NULL,
};
...
static int schedpart_getlist(PROCESS* prp, part_list_t* spart_list, int n, schedpart_flags_t flags, struct _cred_info* cred)
{
    ...
    for (i = 0; i < n_sparts; i++) {
        ...
        if ((flags & schedpart_flags_t_GETLIST_INHERITABLE) && (sp_node->flags & part_flags_NO_INHERIT)) {
            ++filtered;
            continue;
        }
