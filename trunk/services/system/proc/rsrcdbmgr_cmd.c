...
static int do_create_destroy(int (*getfunc)(rsrc_alloc_t* alloc, pid_t pid), int (*restorefunc)(rsrc_alloc_t* alloc, pid_t pid),
...
int rsrcdbmgr_handler(message_context_t* mctp, int code, unsigned flags, void* handle)
{
    ...
    switch (cmd) {
        case RSRCDBMGR_REQ_CREATE:
            ...
            ret = do_create_destroy(_rsrcdbmgr_create, _rsrcdbmgr_destroy, alloc, msg->i.count, rsrcname, pid);
            break;

        case RSRCDBMGR_REQ_DESTROY:
            // Permanently remove from list
            ret = do_create_destroy(_rsrcdbmgr_destroy, _rsrcdbmgr_create, alloc, msg->i.count, rsrcname, pid);
            break;
        ...
        case RSRCDBMGR_REQ_COUNT:
        case RSRCDBMGR_REQ_QUERY_NAME:
            ret = _rsrcdbmgr_query(rsrcname, msg->i.pid, (cmd == RSRCDBMGR_REQ_COUNT) ? FLAG_QUERY_COUNT : 0, msg->i.index, alloc,
                                   msg->i.count * sizeof(*alloc), NULL);
...
int rsrcdbmgr_proc_interface(void* list, int count, int cmd)
{
    ...
    switch (cmd & RSRCDBMGR_REQ_MASK) {
        case RSRCDBMGR_REQ_CREATE:
            ret = do_create_destroy(_rsrcdbmgr_create, _rsrcdbmgr_destroy, list, count, NULL, SYSMGR_PID);
            break;
        case RSRCDBMGR_REQ_DESTROY:
            ret = do_create_destroy(_rsrcdbmgr_destroy, _rsrcdbmgr_create, list, count, NULL, SYSMGR_PID);
            break;
        case RSRCDBMGR_REQ_DETACH:
...
int rsrcdbmgr_proc_query(rsrc_alloc_t* list, int count, int start, int flags)
{
    ...
    pthread_mutex_lock(&g_rsrc_mutex);
    ret = _rsrcdbmgr_query(rsrcdbmgr_id2name(flags), 0, 0, start, list, count * sizeof(*list), NULL);
    pthread_mutex_unlock(&g_rsrc_mutex);
    return ret;
}
