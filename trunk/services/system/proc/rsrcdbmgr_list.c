...
#define OVERLAPE(s1, e1, s2, e2) (CONTAINSE((s1), (e1), (s2)) || CONTAINSE((s2), (e2), (s1)))
...
*/
static int16_t find_range_priority(rsrc_root_node_t* root, uint64_t start, uint64_t end, unsigned flags)
{
    ...
    for (create = root->created; create; create = create->next) {
        ...
        if ((create->start <= start && start <= create->end) || (start <= create->start && create->start <= end)) {
...
*/
static int find_next_priority(rsrc_root_node_t* root, uint64_t start, uint64_t end, int backward, uint64_t* value)
{
    rsrc_create_t* create;
    uint16_t curpriority, priority;

    curpriority = find_range_priority(root, start, end, 0);
    *value = (backward) ? ((start == 0) ? 0 : start - 1) : ((end == ULONGLONG_MAX) ? ULONGLONG_MAX : end + 1);
    ...
    for (create = root->created; create; create = create->next) {
        ...
        if ((create->start <= start && start <= create->end) || (start <= create->start && create->start <= end)) {
            ...
            if (priority >= curpriority && nodeprio >= curpriority) {
                *value = (backward) ? __min(*value, create->start - 1) : __max(*value, create->end + 1);
            }
        }
    }
...
#define BLOCK_SHARE_COMPATABLE(blk, flags, owner) \
    (((blk)->owner == owner) || ((blk)->owner && (blk)->flags & RSRC_FIND_SHARE && (flags)&RSRC_FIND_SHARE))

int rsrc_find_range(rsrc_root_node_t* root, uint64_t length, uint64_t* start, uint64_t* end, uint64_t alignment, pid_t owner,
                    unsigned flags, int* priority)
{
    ...
    restart:
        ...
        if (BLOCK_SHARE_COMPATABLE(curblock, flags, owner) && tmp_start <= tmp_end && length <= ((tmp_end - tmp_start) + 1)) {
            ...
            if (  /*extraamount && We iterate below ... so always check */
                (tmp_start < __max(curblock->start, limit_start) || tmp_end > __min(curblock->end, limit_end))) {
...
*/
rsrc_block_t* rsrc_insert_block(rsrc_block_t** head, rsrc_block_t* curblk, uint64_t start, uint64_t end, pid_t owner, unsigned flags,
                                int after)
{
    rsrc_block_t* newblk;
...
#define JOIN_COMPATIBLE(blk, pid, flags) \
    ((blk)->owner == (pid) && !((blk)->flags & RSRCDBMGR_FLAG_SHARE) && !(flags & RSRCDBMGR_FLAG_SHARE))

/*TODO:
  We need to roll these block_add_range* functions into one!
*/
int rsrc_block_add_range_pid(rsrc_root_node_t* root, uint64_t newstart, uint64_t newend, pid_t pid, unsigned flags)
{
    ...
    for (curblk = root->active; curblk; curblk = curblk->next) {
        if (newstart <= curblk->end + 1) {
            ...
            overlaps = OVERLAPE((newstart != 0) ? newstart - 1 : newstart, (newend != ULONGLONG_MAX) ? newend + 1 : newend, curblk->start,
                                curblk->end);
...
/*
 ...
 function.
*/
int rsrc_block_add_range(rsrc_root_node_t* root, uint64_t newstart, uint64_t newend, pid_t pid, unsigned flags)
{
    rsrc_block_t *curblk, *absorb;
...
*/
int _rsrc_block_limit_op(rsrc_root_node_t* root, uint64_t start, uint64_t end, void* odata,
                         int (*overlap)(rsrc_root_node_t* root, uint64_t start, uint64_t end, void* data), void* nodata,
...
int _rsrc_block_search_op(rsrc_request_t* rsrc, pid_t pid, unsigned flags, int* priority)
{
    ...
    */
    ...
    if (flags & FLAG_ATTACH_DONTLOOK) {
        ...
        for (target = root->created; target && lastprio >= node->priority; target = target->next) {
            ...
            if (curvalid && target->start && target->start - 1 > curend) {
                ...
                if (rsrc_find_range(root, rsrc->length, &curstart, &curend, alignment, frompid, rsrc->flags, &curprio) != -1) {
                    if (curprio < lastprio || (rsrc->flags & RSRCDBMGR_FLAG_TOPDOWN && curprio <= lastprio)) {
    ...
    if (flags & FLAG_ATTACH_RMTOPID) { /* Remove and assign to pid */
        ...
        (void)rsrc_block_add_range_pid(root, lastmatch.start, lastmatch.end, pid, rsrc->flags & RSRCDBMGR_FLAG_SHARE);
...
int _rsrcdbmgr_destroy(rsrc_alloc_t* rsrc, pid_t pid)
{
    ...
    */
    ...
    while ((target = *prev) && target->start <= rsrc->end) {
        if (target->node != node || target->creator != pid || !OVERLAPE(target->start, target->end, rsrc->start, rsrc->end)) {
            prev = &(*prev)->next;
            continue;
        }
...
int _rsrcdbmgr_query(const char* name, pid_t pid, unsigned flags, int start, rsrc_alloc_t* array, uint32_t nbytes, char** newname)
{
    rsrc_root_node_t* root;
    rsrc_block_t* blockp;
    int i, total;
