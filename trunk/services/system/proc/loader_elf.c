...
int elf_load(int fd, const char* path, struct loader_startup* lsp, struct stat* statlocal, struct inheritance* parms)
{
    Elf32_Ehdr ehdr;
    ...
    if (i != sizeof ehdr || memcmp(ehdr.e_ident, ELFMAG, SELFMAG) || ehdr.e_ident[EI_DATA] != ELFDATANATIVE
            ...
            case PT_LOAD:
                ...
                if (lsp != NULL) {
                    ...
                    if (addr == MAP_FAILED) {
                        ...
                        addr = mmap((void*)phdr->p_vaddr, phdr->p_memsz, prot | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_ELF | MAP_FIXED,
                                    NOFD, 0);
                        if (addr == MAP_FAILED) {
                            return errno;
                        }
                        ...
                        if (mem_end > fil_end) {
                            ...
                            if (mmap((void*)fil_end, mem_end - fil_end, prot, (MAP_FIXED | MAP_ANON) | (flags & MAP_TYPE), NOFD, 0)
                                == MAP_FAILED) {
                                return errno;
                            }
                        }
                    ...
                    (void)memmgr.mmap(NULL, (phdr->p_vaddr), (phdr->p_memsz), prot, MAP_PHYS + MAP_SHARED + MAP_SYSRAM, NULL, 0, 0, 0, NOFD,
                        ...
                        if (segaddr->sr_un.sr_addr.sra_skip == 0) {
                            ...
                            for (ent = segaddr->sr_un.sr_addr.sra_index; ent < segaddr[1].sr_un.sr_addr.sra_index; ent++) {
                                ...
                                if (ent >= entend) {
                                    ...
                                    num = min(phdr->p_filesz - ent * sizeof *sr, sizeof buffer - (segent - sr) * sizeof *sr);
    ...
    if (lsp && interp) {
        ...
        for (i = 0, phdr = phdrs; i < ehdr.e_phnum; i++, phdr++) {
            if (phdr->p_type == PT_LOAD) {
                ...
                if (mem_end > fil_end) {
                    // allocate remainder of pages
                    if (mmap((void*)fil_end, mem_end - fil_end, prot, (MAP_ANON | MAP_FIXED) | (flags & MAP_TYPE), NOFD, 0) == MAP_FAILED) {
                        int orig_errno = errno;
