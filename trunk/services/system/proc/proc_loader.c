...
static int loader_fd(struct loader_context* lcp)
{
    ...
    enum { _FD_SPAWN_LIST, _FD_FORK, _FD_SPAWN_NOCLOEXC, _FD_SPAWN_REMOTE, _FD_SPAWN_QUERY } type;
    io_dup_t msgdup;
    ...
    if ((lcp->state & LC_STATE_MASK) != LC_FORK) {
        ...
        if (ND_NODE_CMP(lcp->pnode, ND_LOCAL_NODE) == 0) {
            ...
            nfds = p->nfds;
            flags = p->flags;
            pfdinfo = (struct _proc_spawn_fd_info*)(((uintptr_t)p + sizeof(*p) + p->root_len + p->cwd_len + sizeof(_Int32t) - 1)
                                                    & ~(sizeof(_Int32t) - 1));
        }
    } else {
        nfds = i_nfds = 0;
                ...
                if (nfds-- <= 0) {
                    ...
                    /* read additional fd info */
                    {
                        ...
                        if (i_nfds) {
                            ...
                            SETIOV(lcp->iov + 1, (char*)&lcp->msg.spawn.i + sizeof lcp->msg.spawn.i + fd * sizeof(_Int32t),
                                   spawnfd.i.nfds * sizeof(_Int32t));
...
static void loader_args(struct loader_context* lcp, const char* p, const char* exefile, char* interp_name, char* interp_arg)
{
    char* aux_exefile;
...
#error STACK_GROWS_UP not supported
    ...
    arg = (const char**)((((uintptr_t)strings & ~(sizeof *arg - 1))
                          - (ninterp + lcp->msg.spawn.i.nargv + lcp->msg.spawn.i.narge + naux * (sizeof *lcp->start.aux / sizeof *arg) + 3)
                                * sizeof *arg)
                         & ~(STACK_ALIGNMENT - 1));
...
static int open_exe(const char* name, struct stat* statp)
{
     ...
     */
    if ((statp->st_mode & (S_ISUID | S_ISGID)) && _devctl(fd, DCMD_ALL_GETMOUNTFLAGS, &flags, sizeof(flags), NULL) == EOK
        && (flags & _MOUNT_NOSUID)) {
...
void* loader_load(void* parm)
{
    ...
    if (_mmap2((void*)lcp->start.stackaddr, lcp->start.stacksize + guardpagesize, PROT_READ | PROT_WRITE,
               MAP_PRIVATE | MAP_ANON | MAP_STACK | MAP_LAZY, NOFD, guardpagesize, __PAGESIZE, prealloc, &base, &size)
        == MAP_FAILED) {
