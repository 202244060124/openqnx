...
int _rsrc_minor(const char* name, int* major, int* minor_request, int flags)
{
    ...
    if (flags & RSRCDBMGR_REQ_ATTACH) {
        if (index >= head->minors->count) {
            ...
            if (!(head->minors = _srealloc(head->minors, (head->minors->count - 1) + sizeof(*head->minors), index + sizeof(*head->minors)))) {
                head->minors = tmp;
                return ENOMEM;
            }
...
int rsrcmgr_handle_minor(resmgr_context_t* ctp, rsrc_cmd_t* msg, rsrc_minor_request_t* data)
{
    ...
    if (i >= list_array->dev_count) {
        ...
        list_array->dev_list =
            _srealloc(list_array->dev_list, list_array->dev_count * sizeof(dev_t), (list_array->dev_count + GROW_SIZE) * sizeof(dev_t));
