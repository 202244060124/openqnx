...
static void trace(PROCESS* prp, siginfo_t* info)
{
    ...
    char* p;
    int sig;
    static const char sigstrs[][5] = {"HUP",  "INT",  "QUIT", "ILL",  "TRAP", "ABRT", "EMT",  "FPE",  "KILL", "BUS", "SEGV",
                                      "SYS",  "PIPE", "ALRM", "TERM", "USR1", "USR2", "CHLD", "PWR ", "WNCH", "URG", "POLL",
                                      "STOP", "TSTP", "CONT", "TTIN", "TTOU", "VTAL", "PROF", "XCPU", "XFSZ"};
            ...
            switch (sig) {
                ...
                case SIGBUS:
                    ...
                    if (_dladdr(info->si_fltip, &sym) && sym.dli_sname) {
                        ...
                        p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, "(%s@%s+0x%x)", sym.dli_fname, sym.dli_sname,
