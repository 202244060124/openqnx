...
static const resmgr_io_funcs_t proc_wait_funcs = {
    _RESMGR_IO_NFUNCS, 0, 0, 0, 0, 0, 0, wait_unblock,
};
...
int procmgr_wait_check(PROCESS* prp, PROCESS* parent, struct wait_entry* wap, int match)
{
    ...
    if (match) {
         ...
         */
        match &= wap->options;
        if (wap->idtype == P_ALL && (prp->sibling || parent->child != prp) && sigismember(&parent->sig_ignore, SIGCHLD)) {
            match &= ~WEXITED;
        }
        /*
         ...
         */
        if ((wap->options & WTRAPPED)
            && (prp->flags & (_NTO_PF_DEBUG_STOPPED | _NTO_PF_PTRACED)) == (_NTO_PF_DEBUG_STOPPED | _NTO_PF_PTRACED)) {
            match = WTRAPPED;
        } else if ((wap->options & WEXITED) && (prp->flags & _NTO_PF_WAITINFO)) {
            match = WEXITED;
        } else if ((wap->options & WCONTINUED) && (prp->flags & _NTO_PF_CONTINUED)) {
            match = WCONTINUED;
        } else if ((wap->options & WUNTRACED) && (prp->flags & _NTO_PF_STOPPED) && prp->siginfo.si_signo != 0) {
            match = WUNTRACED;
        }
    }
...
int procmgr_wait(resmgr_context_t* ctp, proc_wait_t* msg)
{
    ...
    if (ND_NODE_CMP(ctp->info.nd, ND_LOCAL_NODE) != 0) {
        ...
        for (child = prp->child; child; child = child->sibling) {
            ...
            /* security check */
            dst = &child->cred->info;
            if (!(src->euid == 0 || src->ruid == dst->ruid || src->ruid == dst->suid || src->euid == dst->ruid || src->euid == dst->suid)) {
