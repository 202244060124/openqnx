...
int procmgr_stack(message_context_t* ctp, int code, unsigned flags, void* handle)
{
    ...
    if ((prp = proc_lookup_pid(pid))) {
        ...
        if (attr.__guardsize == ~0U) {
            ...
            status = memmgr.mmap(prp, 0, attr.__stacksize + attr.__guardsize, thread_stack_prot, mmap_flags, 0, attr.__guardsize,
                                 __PAGESIZE, attr.__prealloc, NOFD, &attr.__stackaddr, &attr.__stacksize, mpid);
            if (status == ENOMEM) {
                status = EAGAIN;
            }
        }
