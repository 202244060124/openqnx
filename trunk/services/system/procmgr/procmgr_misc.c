...
struct loader_context* procmgr_context_alloc(unsigned msgsize, int state)
{
    ...
    if (lcp == NULL) {
        ...
        r = memmgr.mmap(NULL, 0, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANON, 0, 0, __PAGESIZE, 0, NOFD, (void*)&lcp,
                        &size, mempart_getid(procnto_prp, sys_memclass_id));
        if (r != EOK) {
            return NULL;
        }
        pthread_mutex_lock(&procmgr_context_alloc_mutex);
