...
int procmgr_spawn(resmgr_context_t* ctp, void* vmsg, proc_create_attr_t* partlist)
{
    ...
    switch (msg->spawn.i.subtype) {
        case _PROC_SPAWN_START: {
            ...
            off = msgsize = sizeof(struct _proc_spawn) + msg->spawn.i.searchlen + msg->spawn.i.pathlen + msg->spawn.i.nfds * sizeof(int32_t)
                            + msg->spawn.i.nbytes;
        ...
        case _PROC_SPAWN_FD: {
            ...
            max = min(max, (ctp->msg_max_size - ((uintptr_t)ctp->iov[1].iov_base - (uintptr_t)&msg->spawn_fd))
                               / sizeof(struct _proc_spawn_fd_info));
            nfds = get_fdinfo(msg->spawn_fd.i.ppid, ctp->info.nd, msg->spawn_fd.i.base, max, ctp->iov[1].iov_base, nfds,
                              (int*)((char*)&msg->spawn_fd + sizeof msg->spawn_fd.i), msg->spawn_fd.i.flags);
            if (nfds == -1) {
                return errno;
            }
        ...
        case _PROC_SPAWN_REMOTE: {
            ...
            nfds = get_fdinfo(ctp->info.pid, parms->nd, 0, max, (struct _proc_spawn_fd_info*)ptr, msg->spawn.i.nfds,
...
struct loader_context* procmgr_exec(resmgr_context_t* ctp, void* msg, PROCESS* prp)
{
    ...
    if (MsgInfo(lcp->rcvid, &ctp->info) == -1 || !prp->child || (prp->pid != lcp->ppid) || !(child = proc_lock_pid(lcp->pid))
