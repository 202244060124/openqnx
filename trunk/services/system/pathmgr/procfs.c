 ...
 */
static int return_a_link(resmgr_context_t* ctp, struct _io_connect_link_reply* link_reply, char* path, char* append, int eflag,
                         int newlookup, int addslash, struct _io_connect_entry* entry)
{
    ...
    /*
     ...
     {path}
    */
    return (_RESMGR_PTR(ctp, link_reply, sizeof(*link_reply) + len + ((entry) ? sizeof(*entry) : 0)));
}
    ...
    */
    ...
    if ((type & PROCFS_FLAG_MOUNT) && result && *result) {
        ...
        for (obj = nop->object; obj; obj = obj->hdr.next) {
            ...
            if (obj->server.nd == entry.nd && obj->server.pid == entry.pid && obj->server.chid == entry.chid
                && obj->server.handle == entry.handle && obj->server.file_type == entry.file_type) {
                entry.key = 0;
                break;
            }
        }
...
#if 1
        if ((obj->hdr.type == OBJECT_NAME) || (obj->hdr.flags & PATHMGR_FLAG_FTYPEONLY) && msg->connect.file_type != entry.file_type) {
...
#else
        if ((msg->connect.ioflag & _IO_FLAG_MASK) == 0 && *cresult == '\0' && entry.file_type != _FTYPE_ANY) {
            if (obj->hdr.flags & PATHMGR_FLAG_DIR) {
                msg->connect.mode |= S_IFDIR;
            }
            ret = EOK;
        } else if (msg->connect.file_type != entry.file_type && (obj->hdr.flags & PATHMGR_FLAG_FTYPEONLY)) {
        ...
        if (obj) {
            ...
            ret = return_a_link(ctp, &msg->link_reply, nop->name, cresult, msg->connect.eflag, 1, 1, NULL);
        }
        if (handle) {
            *handle = obj;
        }
...
static int proc_pathmgr_read(resmgr_context_t* ctp, io_read_t* msg, struct procfs_ocb* ocb)
{
    ...
    nbytes = min(msg->i.nbytes, ctp->msg_max_size - ((offsetof(struct dirent, d_name) + max_npc_size + 7) & ~7));
    ...
    if (ocb->offset & TOP_BIT) {
        ...
        for (tmpnode = nop->child; tmpnode; tmpnode = tmpnode->sibling) {
            if (tmpnode->object) {
                if ((ocb->flags & PROCFS_FLAG_MOUNT) && tmpnode->object->hdr.type != OBJECT_SERVER
...
static int procfs_read(resmgr_context_t* ctp, io_read_t* msg, void* vocb)
{
    ...
    */
    ...
    size = min(msg->i.nbytes, ctp->msg_max_size - ((offsetof(struct dirent, d_name) + INT_STRLEN_MAXIMUM(pid_t) + 1 + 7) & ~7));
    d = (struct dirent*)(void*)msg;
    nbytes = 0;
...
static int memmgr_shared_to_private(PROCESS* prp, uintptr_t addr, size_t size)
{
    ...
    size_t r = memmgr.mapinfo(prp, addr & ~(__PAGESIZE - 1), &info, &dinfo.debug, sizeof(dinfo) - offsetof(procfs_debuginfo, path), NULL,
                              NULL, NULL);
    ...
    status = memmgr.mmap(prp, info.vaddr, /* address */
                         size, (info.flags & PROT_MASK) | PROT_WRITE,
        ...
        default:
            ...
            switch ((unsigned)msg->i.dcmd) {
                ...
                case DCMD_PROC_MAPDEBUG: {
                    ...
                    space = min(ctp->msg_max_size - sizeof(*msg), msg->i.nbytes);
                    CRASHCHECK(space < offsetof(procfs_debuginfo, path));
                    if (memmgr.mapinfo(prp, ioctl->mapdebug.vaddr, NULL, &ioctl->mapdebug, space - offsetof(procfs_debuginfo, path), NULL,
                                       NULL, NULL)
                        == 0) {
                        proc_unlock_adp(prp);
                        return proc_error(ENXIO, prp);
                ...
                case DCMD_PROC_IRQS: {
                    ...
                    do {
                        ...
                        nbytes = 0;
                        num = (ctp->msg_max_size - sizeof *msg) / sizeof *p;
                        while (num && ((entryp = QueryObject(_QUERY_INTERRUPT, id, 0, 0, &id, &entry, sizeof(entry))) != NULL)) {
                ...
                case DCMD_PROC_TIMERS: {
                    ...
                    do {
                        ...
                        nbytes = 0;
                        num = (ctp->msg_max_size - sizeof *msg) / sizeof *p;
                        while (num-- && ((id = TimerInfo(prp->pid, id, _NTO_TIMER_SEARCH, &p->info)) != -1)) {
                            p->id = id;
                            if (p->info.event.sigev_notify == SIGEV_SIGNAL_CODE
                                && p->info.event.sigev_code
                                       == SI_TIMER) { /* ???? Not safe.  Shouldn't there be a way of remembering this better? */
                ...
                case DCMD_PROC_CHANNELS: {
                    ...
                    do {
                        ...
                        num = (ctp->msg_max_size - sizeof *msg) / sizeof *p;
                        while (num
                               && ((chp = QueryObject(_QUERY_PROCESS, prp->pid, _QUERY_PROCESS_CHANCONS, id, &id, &chan, sizeof(chan)))
                                   != NULL)) {
                    ...
                    if ((NUM_PROCESSORS > 1) && (ioctl->brk.type & _DEBUG_BREAK_EXEC)) {
                        ...
                        if ((info.flags & MAP_TYPE) == MAP_SHARED) {
                            ...
                            if ((info.flags & PROT_WRITE) ||
                                ...
                                (proc_rlock_promote_adp(prp) == -1) || memmgr_shared_to_private(prp, ioctl->brk.addr, __PAGESIZE) != EOK) {
                                proc_unlock_adp(prp);
                                return proc_error(EINVAL, prp);
                            }
                        }
                ...
                case DCMD_PROC_GET_BREAKLIST: {
                    ...
                    do {
                        ...
                        if (resmgr_msgwrite(ctp, breaklist.breakpoints, breaklist.count * sizeof(*breaklist.breakpoints),
...
static const resmgr_io_funcs_t procfs_io_funcs = {_RESMGR_IO_NFUNCS, procfs_read,    procfs_write,
                                                  procfs_close_ocb,  procfs_stat,    0,
                                                  procfs_devctl,     procfs_unblock, 0,
                                                  procfs_lseek};
...
{
    resmgr_attach(dpp, NULL, "/proc", _FTYPE_ANY, _RESMGR_FLAG_DIR, &procfs_connect_funcs, &procfs_io_funcs, 0);
