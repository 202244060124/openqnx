...
int reply_symlink(resmgr_context_t* ctp, unsigned eflag, struct _io_connect_link_reply* linkp, struct symlink_object* symlinkp,
...
/*
 ...
 server _only_.
*/
int _netmgr_connect(int base, const char* path, mode_t mode, unsigned oflag, unsigned sflag, unsigned subtype, int testcancel,
                    unsigned accessl, unsigned file_type, unsigned extra_type, unsigned extra_len, const void* extra, unsigned response_len,
                    void* response, int* status)
{
    ...
    ret = _connect_entry(base, path, mode, oflag, sflag, subtype, testcancel, accessl, file_type, extra_type, extra_len, extra,
                         response_len, response, status, &entry, 1);
...
static int pathmgr_fdinfo(resmgr_context_t* ctp, io_fdinfo_t* msg, void* vocb)
{
    ...
    if (pathmax) {
        ...
        if ((len = netmgr_ndtostr((flags & _FDINFO_FLAG_LOCALPATH) ? ND2S_LOCAL_STR | ND2S_DIR_SHOW : ND2S_DIR_SHOW, obp->server.nd, path,
                                  pathmax))
            <= 1) {
            len = 1;
            *path = '\0';
...
static const resmgr_io_funcs_t resmgrlink_io_funcs = {_RESMGR_IO_NFUNCS, 0, 0, pathmgr_close_ocb, pathmgr_stat, 0, 0, 0, 0, 0, 0, 0, 0, 0,
                                                      pathmgr_fdinfo};
...
static int pathmgr_resmgrlink(resmgr_context_t* ctp, io_link_t* msg, void* handle, struct _io_resmgr_link_extra* extra)
{
    OBJECT* obp;
    PROCESS* prp;
...
int pathmgr_node2fullpath(NODE* node, char* path, int pathmax)
{
    ...
    /* If local path requested or if netmgr fails, don't include network component */
    len = netmgr_ndtostr(ND2S_DIR_SHOW, node->object ? node->object->server.nd : ND_LOCAL_NODE, path, pathmax);
