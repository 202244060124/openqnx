...
*/
static int image_getdir_children(union image_dirent* startdir, char* path, char*** children, uint16_t* childcount, uint16_t* index)
{
    union image_dirent* dir;
    int len, ncount;
    char** names;
    ...
    for (dir = startdir; dir->attr.size; dir = (union image_dirent*)((char*)dir + dir->attr.size)) {
        ...
        */
        for (i = 0; i < ncount && names[i]; i++) {
            if (strncmp(names[i], entry_name, entry_len) == 0 && (names[i][entry_len] == '/' || names[i][entry_len] == '\0')) {
                entry_name = NULL;
                break;
            }
        }
...
static union image_dirent* image_lookup(struct image_data* image, struct _io_connect* connect, int* state)
{
    ...
    for (dir = image->dir; dir->attr.size; dir = (union image_dirent*)((unsigned)dir + dir->attr.size)) {
        ...
        if (S_ISLNK(dir->attr.mode) && entry_len < len && name[entry_len] == '/' && strncmp(entry_name, name, entry_len) == 0) {
...
static int image_link_redirect(resmgr_context_t* ctp, io_open_t* msg, struct image_symlink* symlinkp, int redirect)
{
    int len, eflag;
...
static int image_open(resmgr_context_t* ctp, io_open_t* msg, void* handle, void* extra)
{
    ...
    if ((dire = image_lookup(image, &msg->connect, &notmatch))) {
        ...
        if (notmatch != LOOKUP_INT_DIR && S_ISLNK(dire->attr.mode)) {
            if ((notmatch == LOOKUP_INT_LNK) || !S_ISLNK(msg->connect.mode) || (msg->connect.subtype == _IO_CONNECT_READLINK)
...
//
static int image_mount(resmgr_context_t* ctp, io_mount_t* msg, void* handle, io_mount_extra_t* extra)
{
    char* options;
    char* p;
    void* ifs_image;
...
{
    return resmgr_attach(dpp, NULL, NULL, _FTYPE_MOUNT, _RESMGR_FLAG_DIR | _RESMGR_FLAG_FTYPEONLY, &image_connect_funcs, &image_io_funcs,
                         NULL);
}
...
void* imagefs_mount(paddr_t paddr, unsigned size, unsigned offset, unsigned flags, struct node_entry* root, const char* mountpoint)
{
    ...
    // FUTURE: as well.
    r = memmgr.mmap(NULL, 0, size, PROT_WRITE | PROT_READ, MAP_SHARED, obp, pg_offset, 0, 0, NOFD, &addr, &size, obp->hdr.mpid);
    if ((r == ENOTSUP) && (mountpoint != NULL)) {
        ...
        in_user_space = 1;
        ProcessBind(SYSMGR_PID);
        r = memmgr.mmap(sysmgr_prp, 0, size, PROT_WRITE | PROT_READ, MAP_SHARED, obp, pg_offset, 0, 0, NOFD, &addr, &size, obp->hdr.mpid);
    }
    if (r != EOK)
        goto fail2;
...
#define NATIVE_ENDIAN IMAGE_FLAGS_BIGENDIAN
    ...
    if (resmgr_attach(dpp, NULL, mountpoint, _FTYPE_ANY, flags | _RESMGR_FLAG_DIR, &image_connect_funcs, &image_io_funcs, image) == -1) {
        r = errno;
        goto fail5;
    }
...
fail3:
    (void)memmgr.munmap(in_user_space ? sysmgr_prp : NULL, (uintptr_t)addr, size, 0, obp->hdr.mpid);
    ...
    *pobp = image->obp;
    ...
    if (msg != NULL) {
        ...
        if (msg->i.flags & MAP_ELF) {
            ...
            if (memcmp(ehdr->e_ident, ELFMAG, SELFMAG) != 0 || ehdr->e_ident[EI_DATA] != ELFDATANATIVE
            ...
            if (phdr && (UNALIGNED_RET32(&phdr->p_flags) & PF_W) && (UNALIGNED_RET16(&ehdr->e_type) == ET_EXEC)) {
