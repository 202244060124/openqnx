...
int pathmgr_resolve_servers(resmgr_context_t* ctp, struct node_entry* node, struct _io_connect* connect, const char* path,
                            struct node_entry* root)
{
    ...
    if (connect->type == _IO_CONNECT && connect->subtype == _IO_CONNECT_LINK && connect->extra_type == _IO_CONNECT_EXTRA_RESMGR_LINK) {
    ...
    reply =
        (char*)connect
        + ((offsetof(struct _io_connect, path) + connect->path_len + connect->extra_len + len + _MALLOC_ALIGN - 1) & ~(_MALLOC_ALIGN - 1));
    ...
    for (n = node;; n = n->parent) {
        ...
        for (o = n->object; o; o = o->hdr.next) {
            ...
            if (local_flags & _PMFLAG_SEEN_OPAQUE) {
                if (o->hdr.type != OBJECT_SERVER || (local_flags & _PMFLAG_SEEN_PATHMGR) || o->server.pid != PATHMGR_PID
                    || o->server.handle != root_id) {
                    continue;
                }
            }
            ...
            if (o->hdr.type == OBJECT_SERVER) {
                if ((!(local_flags & _PMFLAG_IS_LINK) || o->server.pid == PATHMGR_PID)
                    ...
                    && (o->server.pid != ctp->info.pid || ND_NODE_CMP(o->server.nd, ctp->info.nd) || (o->hdr.flags & PATHMGR_FLAG_SELF))) {
