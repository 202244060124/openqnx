...
static int mem_read_write(resmgr_context_t* ctp, resmgr_iomsgs_t* msg, struct mem_ocb* ocb)
{
    ...
    if (nbytes) {
        ...
        status = memmgr.mmap(proc, 0, nbytes, PROT_READ | PROT_WRITE, flags, obp, *pos, 0, nbytes, NOFD, &addr, &size, mpid);
        if (obp != NULL)
...
static const resmgr_io_funcs_t mem_io_funcs = {_RESMGR_IO_NFUNCS, mem_read,  mem_write, mem_close_ocb, mem_stat, 0, mem_devctl, 0,        0,
                                               mem_lseek,         mem_chmod, mem_chown, mem_utime,     0,        0, 0,          mem_space};
...
int mem_open(resmgr_context_t* ctp, io_open_t* msg, void* handle, void* extra)
{
    ...
    if (object) {
        ...
        if (msg->connect.ioflag & O_CREAT && !object->mem.pm.mode) {
            ...
            if (ret != 0) {
                ret = devmem_check_perm(ctp, object->mem.pm.mode, object->mem.pm.uid, object->mem.pm.gid, ret);
            }
            if (ret != 0) {
                return ret;
            }
...
static const resmgr_connect_funcs_t mem_connect_funcs = {_RESMGR_CONNECT_NFUNCS, mem_open};
...
static const resmgr_connect_funcs_t shmem_connect_funcs = {_RESMGR_CONNECT_NFUNCS, shmem_open};
...
static const resmgr_io_funcs_t tymem_io_funcs =
    {_RESMGR_IO_NFUNCS, 0, 0, mem_close_ocb, mem_stat, 0, mem_devctl, 0, 0, 0, mem_chmod, mem_chown, mem_utime, 0, 0, 0, 0};
