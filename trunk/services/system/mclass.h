...
extern void (*memclass_event)(struct apmmgr_attr_s* attr, memclass_evttype_t evtype, memclass_sizeinfo_t* cur, memclass_sizeinfo_t* prev);
...
#define MEMCLASS_PID_USE(p, mc, sz)               \
    do {                                          \
        PROCESS* _prp = (p);                      \
        memsize_t _size = (sz);                   \
        if ((_prp != NULL) && (_size > 0)) {      \
            _memclass_pid_use(_prp, (mc), _size); \
        }                                         \
    } while (0)
#define MEMCLASS_PID_FREE(p, mc, sz)               \
    do {                                           \
        ...
        memsize_t _size = (sz);                    \
        if ((_prp != NULL) && (_size > 0)) {       \
            _memclass_pid_free(_prp, (mc), _size); \
        }                                          \
    } while (0)
#else /* NDEBUG */
#define MEMCLASS_PID_USE(p, mc, sz)               \
...
#define MEMCLASS_PID_USE_INKER(_pid, _mpid, sz) \
    if ((sz) > 0) {                             \
        MemclassPidUse((_pid), (_mpid), (sz));  \
    }
#define MEMCLASS_PID_FREE_INKER(_pid, _mpid, sz) \
    if ((sz) > 0) {                              \
        MemclassPidFree((_pid), (_mpid), (sz));  \
    }
#else
#define MEMCLASS_PID_USE_INKER(_pid, _mpid, sz)
#define MEMCLASS_PID_FREE_INKER(_pid, _mpid, sz)
