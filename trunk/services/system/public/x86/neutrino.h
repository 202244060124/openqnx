...
#pragma aux ClockCycles = ".386p" \
                          "db 0fh,31h" modify exact nomemory[eax edx];

#pragma aux __inline_InterruptEnable = "sti" \
                                       "mov eax,eax" parm[] modify exact[];

#pragma aux __inline_InterruptDisable = "cli" parm[] modify exact[];

extern unsigned __cpu_flags;
#pragma aux __inline_InterruptUnlock = "test    dword ptr __cpu_flags,0x8000" \
                                       "je        skip"                       \
                                       "db 0x0f, 0xae, 0xf0"                  \
                                       "skip:"                                \
                                       "mov dword ptr [eax],0"                \
                                       "sti" parm[eax] modify exact[];

#pragma aux __inline_InterruptLock = "cli"               \
                                     "L1:"               \
                                     "mov    edx,1"      \
                                     "xchg    edx,[eax]" \
                                     "test    edx,edx"   \
                                     ...
                                     "byte 0xf3,0x90"    \
                                     "jmp    L1"         \
                                     "L2:" parm[eax] modify exact[edx];

#pragma aux __inline_InterruptStatus = ".386"    \
                                       "pushfd"  \
                                       "pop eax" \
                                       "and eax,0x200" parm nomemory[] modify exact[eax] value[eax]

#pragma aux __inline_DebugBreak = "int 0x03" parm nomemory[] modify nomemory exact[];

#pragma aux __inline_DebugKDBreak = "int 0x20" parm nomemory[] modify nomemory exact[];

#pragma aux __inline_DebugKDOutput = "int 0x21" parm nomemory[eax][edx] modify nomemory exact[];

#elif defined(__GNUC__) || defined(__INTEL_COMPILER)
#define ClockCycles()                                   \
    ({                                                  \
        register _Uint64t __cycles;                     \
        __asm__ __volatile__("rdtsc" : "=A"(__cycles)); \
        __cycles;                                       \
    })
...
#define __inline_InterruptEnable() __asm__ __volatile__("sti" : : : "memory")
...
#define __inline_DebugKDOutput(__text, __len) ((void)({ __asm__ __volatile__("int $0x21" : : "a"(__text), "d"(__len)); }))
