...
struct thread_entry {     // Also used for vthread_entry
    ...
    uint64_t timestamp_last_block; /* set to copy of snap_time() anytime this thread blocks. not valid if state is STATE_READY or
                                      STATE_RUNNING */
    PROCESS* aspace_prp;
...
enum { SYNCEVENT_SUBTYPE_EVENT, SYNCEVENT_SUBTYPE_PRIORITYCEILING, SYNCEVENT_SUBTYPE_CLOCKID };
