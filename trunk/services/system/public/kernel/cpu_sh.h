...
#define CPU_VADDR_IN_RANGE(v)               ((uintptr_t)(v) >= SH_P1BASE && (uintptr_t)(v) < SH_P3BASE)
...
#define SYSP_ADDCOLOR(addr, color) \
    ((SYSP_GETCOLOR(addr) > color) ? ((addr + SYSP_SIZECOLOR) & ~SYSP_COLOR_MASK) | color : (addr & ~SYSP_COLOR_MASK) | color)
