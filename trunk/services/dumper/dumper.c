...
#define OFFENDING_THREAD(base, size, mapinfoptr) ((mapinfoptr)->vaddr >= base && (mapinfoptr)->vaddr < (base + size))
...
static int dump_seek(FILE* core_fp, off_t off)
{
    ...
    else if (gzseek((gzFile)core_fp, off, SEEK_SET) == -1) {
...
void fixup_stack_boundary(procfs_status* thread, procfs_mapinfo* mapinfos, int n)
{
    ...
    for (i = 0; i < n; i++) {
        if (thread->sp >= mapinfos[i].vaddr && thread->sp < mapinfos[i].vaddr + mapinfos[i].size) {
            ...
            dprintf(("Adjusting %lld bytes @ %#llx->%#llx to %lld bytes @ %#llx->%#llx\n", mapinfos[i].size, mapinfos[i].vaddr,
                     mapinfos[i].vaddr + mapinfos[i].size, size, vaddr, vaddr + size));
...
void slog_tid(procfs_status* status, const char* path)
{
    switch (status->why) {
        ...
        case _DEBUG_WHY_FAULTED:
            slogf(_SLOG_SETCODE(_SLOGC_DUMPER, 0), _SLOG_INFO, "run fault pid %d tid %d signal %d code %d ip %#llx %s", (int)status->pid,
                  (int)status->tid, (int)status->info.si_signo, (int)status->info.si_code, (uint64_t)status->ip, path ?: "");
            break;
        default:
            slogf(_SLOG_SETCODE(_SLOGC_DUMPER, 0), _SLOG_INFO, "run fault pid %d tid %d why %d what %d ip %#llx %s", (int)status->pid,
                  (int)status->tid, (int)status->why, (int)status->what, (uint64_t)status->ip, path ?: "");
            break;
    }
}
...
int elfcore(int fd, FILE* fp, const char* path, long coresize)
{
    ...
    for (seg = 0, i = 0; i < num; i++) {
        if (!(mapinfos[i].flags & PG_HWMAPPED) || (nodumpmem && !(mapinfos[i].flags & MAP_STACK))) {
            continue;
        }
        ...
        if (cur_tid_only && (mapinfos[i].flags & MAP_STACK) && !OFFENDING_THREAD(cur_tid_base, cur_tid_size, &mapinfos[i])) {
            continue;
        }
    ...
    for (i = 0; i < n_ldd_infos; i++) {
        dprintf(("%svaddr=%#llx, offset=%#llx, size=%#llx, flags=%#x\n", ldd_infos[i].flags & PG_HWMAPPED ? "*" : "", ldd_infos[i].vaddr,
                 ldd_infos[i].offset, ldd_infos[i].size, ldd_infos[i].flags));
...
int main(int argc, char* argv[])
{
    ...
    if (-1 == procmgr_daemon(EXIT_SUCCESS, verbose ? PROCMGR_DAEMON_NODEVNULL | PROCMGR_DAEMON_KEEPUMASK : PROCMGR_DAEMON_KEEPUMASK)) {
