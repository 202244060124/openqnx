...
static int get_headers(int fd, Elf32_Ehdr* ehdr, Elf32_Phdr** phdr)
{
    ...
    *phdr = malloc(ehdr->e_phnum * ehdr->e_phentsize);
    ...
    if (read_memory(fd, info.base_address + ehdr->e_phoff, *phdr, ehdr->e_phnum * ehdr->e_phentsize) < ehdr->e_phnum * ehdr->e_phentsize) {
        free(*phdr);
        return -1;
    }
        ...
        if (phdr[n].p_type == PT_DYNAMIC) {
            ...
            do {
                ...
                if (d.d_tag == DT_MIPS_RLD_MAP) {
                    ...
                    /* On MIPS, the DT_MIPS_RLD_MAP entry points to what normally would be the DT_DEBUG pointer, so an extra indirection is
                     * needed */
