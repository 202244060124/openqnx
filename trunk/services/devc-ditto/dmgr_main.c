...
static int path_init(char* devname, dispatch_t* dpp, int cloner_key)
{
    ...
    if ((rcbp->ocache.ca_buf = malloc(obufsize)) == 0 || (rcbp->ccache.ca_buf = malloc(obufsize)) == 0) {
        free(rcbp);
        return errno;
    }
    ...
    if ((rcbp->resid = resmgr_attach(dpp, &res_attr, devname, _FTYPE_ANY, _RESMGR_FLAG_BEFORE, &di_connect_funcs, &di_io_funcs, rcbp))
        == -1) {
        int n = errno;
    ...
    if ((rcbp->resdid = resmgr_attach(dpp, &res_attr, ldevname, _FTYPE_ANY, 0, &di_connect_funcs, &di_io_funcs, rcbp)) == -1) {
        int n = errno;
...
static int mt_mount(resmgr_context_t* ctp, io_mount_t* msg, void* handle, io_mount_extra_t* extra)
{
    ...
    if ((extra->flags & _MFLAG_OCB) && strcmp(extra->extra.srv.type, "clone") && strcmp(extra->extra.srv.type, "ditto"))
        return ENOSYS;
...
int main(int argc, char** argv)
{
    ...
    if (resmgr_attach(dpp, &res_attr, 0, _FTYPE_MOUNT, _RESMGR_FLAG_FTYPEONLY | _RESMGR_FLAG_DIR, &mt_connect_funcs, 0, 0) == -1) {
