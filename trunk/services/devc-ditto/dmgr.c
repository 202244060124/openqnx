...
static int di_bypass(resmgr_context_t* ctp, void* umsg, struct relay_ocb* ocb)
{
    ...
    if (slen > ctp->info.msglen) {
        if (MsgRead(ctp->rcvid, rbuf, slen, 0) == -1 || (n = MsgSend(ocb->realfd, rbuf, slen, rbuf, dlen)) == -1) {
            return errno;
        }
    } else {
...
static int di_open(resmgr_context_t* ctp, io_open_t* msg, void* handle, void* extra)
{
    ...
    o->realfd = realfd;
    o->rcb = rcb;
    if ((o->bpbuf = malloc(rcb->bpbuf_size)) == NULL || (resmgr_open_bind(ctp, o, &di_io_funcs) == -1)) {
        int n = errno;
        free(o);
        close(realfd);
        return n;
