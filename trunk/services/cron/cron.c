...
static char* parse_ctab(char* line, unsigned long* crtime)
{
    ...
    do { /* Iterates through the first 5 of 6 times elements. */
        if (strcmp(field, "*") == 0) {
            ...
            for (;; ++field) { /* comma delimited list */
    ...
    if (dirp = opendir("crontabs/")) {
        ...
        while (dent = readdir(dirp)) { /* Loop through crontabs */
            ...
            while (fgets(entrybuf, TAB_BUFFER_SIZE, fp) != NULL) {
                ...
                if (((job_tmp = malloc(sizeof(cron_job))) == NULL) || ((job_tmp->command = strdup(command)) == NULL)) {
...
long do_jobs()
{
    ...
    for (job_index = cron_jobs; job_index; job_index = job_index->next) {
        ...
        if ((job_index->time[WDAY] & now_time[WDAY]) && (job_index->time[MDAY] & now_time[MDAY]) && (job_index->time[MON] & now_time[MON])) {
            /* Check if this job is scheduled for this min */
            if ((job_index->time[HOUR] & now_time[HOUR])
                && ((job_index->time[MIN_LO] & now_time[MIN_LO]) || (job_index->time[MIN_HI] & now_time[MIN_HI]))) {
                trigger(job_index);
            }
            ...
            if ((nexthr < best_nexthr) || ((nexthr == best_nexthr) && (nextmin < best_nextmin))) {
                best_nexthr = nexthr;
