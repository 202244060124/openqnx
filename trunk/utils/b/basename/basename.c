...
% C
    - return nondirectory portion of pathname(POSIX)
             ...
             % C string[suffix] Note : This will print to stdout the basename portion of string,
    optionally with the suffix removed from the end of it(if the suffix was present)
        .
#endif
    ...
    int main(int argc, char* argv[])
{
    ...
    switch (argc - optind) {
        case 2:
            sfx_str = argv[optind + 1]; /* fall thru */
        case 1:
            src_str = argv[optind];
            break;
        default:
            fprintf(stderr, "basename: invalid number of operands\n");
            inval_usage++;
            break;
    }
 ...
 */
    ...
    for (i = 0, dst_str = src_str; *dst_str; dst_str++)
        if (*dst_str != '/')
            i++;   /* i non-zero if any chars other than / encountered */
    if (!i)
        dst_str--; /* all /s - back off 1 so dst_str = "/" */
    else {
#ifdef DEBUG
        ...
        if (sfx_str != NULL) {
            ...
            if ((end_str > dst_str) && !strcmp(end_str, sfx_str))
                *end_str = 0;
        }
#ifdef DEBUG
